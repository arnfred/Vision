%   [m,ga,b,z,zu,nlZ,Q,T] = DLI(X,y,s2,B,pot,tau,opts,G)
%
...
%         approximation N(u|m,V) to the posterior
%
% See also INFENGINE.M.
%
% (c) by Hannes Nickisch, MPI for Biological Cybernetics, 2011 October 21

function [m,ga,b,z,zu,nlZ,Q,T] = dli(X,y,s2,B,pot,tau, opts,G)

if nargin<6, error('We need at least 6 input arguments'), end
if nargin==6, opts = 1; end
if nargin<8, G = 1; else if numel(G)==0, G = 1; end, end           % set default
...
for i = 1:nout
  ...
  [u,pi,b,phi0] = feval(it,u,pi,b,z,X,y,s2,B,pot,tau,G,innerExact,ninMVM,opts);
  ...
  if cvg || i==nout % converged or last loop
    ...
    m = postmean(X,s2,B,pi,y,b,innerExact,ninMVM,u,output); ga = 1./pi;
    break
  end
end
...
% Inner Loop: Variational Bayesian: penalised least squares
function [u,pi,b,phi0] = innerVB(u,pi,b,z,X,y,s2,B,pot,tau,G,exact,ninMVM,opts)
  ...
  if G==1                    % PLS, no ga dependence since implicitely minimised
    [u,phi_z] = feval(pls,u,X,y,B,pls_opts,s2,'penVB',pot,tau,z);
  else
    [u,phi_z] = feval(pls,u,X,y,B,pls_opts,s2,'penVBNorm',pot,tau,z,G);
  end
...
function [u,pi,b,phi0] = innerEP(u,pi,b,z,X,y,s2,B,pot,tau,G,exact,ninMVM,opts)
  ...
  for jj=1:nsweep
    ...
    u = postmean(X,s2,B,pi,y,b,exact,ninMVM,u,opts.innerOutput); s = B*u; % mean
...
function m = postmean(X,s2,B,pi,y,b,exact,nMVM,u,output)        % posterior mean
  if nargin<10, output = false; end
  ...
  if exact
    if woodbury
      ...
      m = linsolve_woodbury(X,1/s2,pi,X'*y/s2+B'*b);          % Woodbury formula
    else
      if output, fprintf('  Compute exact mean using dense matrices\n'), end
      m = linsolve_full(X,1/s2,B,pi,X'*y/s2+B'*b);         % use the full matrix
    end
  else
    if output, fprintf('  Approximate mean using LCG\n'), end
    m = linsolve_lcg(X,1/s2,B,pi,[X']*y/s2+[B']*b,nMVM,u);
  end
