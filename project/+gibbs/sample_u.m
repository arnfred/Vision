...
function [u u_mu i i_mu psnr psnr_mu r r_mu] = sample_u(mrf, z, start_sample, B, sigma, psnr_fun, psnr_fun_mu)
	...
	[u_mu i_mu r_mu psnr_mu]	= cg_prec(B, z, start_sample / (sigma^2), mrf.epsilon + (1/ sigma^2), psnr_fun_mu);

	% Prepare psnr_fun for u_delta
	psnr_fun_delta				= @(u_cg) psnr_fun(u_mu + u_cg);

	% Calculate the delta of u
	[u_delta i r psnr]			= cg_prec(B, z, rhs, mrf.epsilon + (1/ sigma^2), psnr_fun_delta);
